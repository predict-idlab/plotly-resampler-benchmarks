...
with VizTracer(
        log_gc=False,
        log_async=True,
        output_file=f"{args.save_dir}/bokeh__{args.index}__nb_traces={args.nb_traces}__n={args.n}__n_out={args.n_out}__headless={args.headless}.json",
        max_stack_depth=0,
        plugins=["vizplugins.memory_usage"],
) as viz:
    output_file(filename="bokeh_img.html", title="Static HTML file")
    fig = figure(output_backend="webgl")
    [
        fig.line(x=s.index, y=s, name=str(i))
    ...
    save(fig)
    fr.go_to_page("file://" + os.path.abspath("bokeh_img.html"))
    fr.wait_element("bk-tool-icon-wheel-zoom", wait_time_s=900)

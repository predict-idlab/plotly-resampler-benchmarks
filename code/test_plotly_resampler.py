...
with VizTracer(
        log_gc=False,
        log_async=True,
        output_file=f"{args.save_dir}/plotly_resampler__{args.index}__nb_traces={args.nb_traces}__n={args.n}__n_out={args.n_out}__headless={args.headless}.json",
        max_stack_depth=0,
        plugins=["vizplugins.memory_usage"],
) as viz:
    fig = FigureResampler(go.Figure(), default_n_shown_samples=args.n_out)
    [
        fig.add_trace(go.Scattergl(name=i), hf_x=s.index, hf_y=s)
    ...
    fr.go_to_page()
    fr.wait_element("xy", wait_time_s=600)
